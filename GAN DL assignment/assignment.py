...
class Generator_Model(tf.keras.Model):
    def __init__(self):
        ...
        self.model.add(tf.keras.layers.Dense(512*4*4, activation = "relu"))
        self.model.add(tf.keras.layers.BatchNormalization())
        self.model.add(tf.keras.layers.Conv2DTranspose(filters = 256, kernel_size = 5, stride = 2, padding = "same",
                                              activation = tf.keras.layers.ReLU))
        self.model.add(tf.keras.layers.BatchNormalization())
        self.model.add(tf.keras.layers.Conv2DTranspose(filters = 128, kernel_size = 5, stride = 2, padding ="same",
                                              activation = tf.keras.layers.ReLU))
        self.model.add(tf.keras.layers.BatchNormalization())
        self.model.add(tf.keras.layers.Conv2DTranspose(filters = 64, kernel_size = 5, stride = 2, padding = "same",
                                              activation = tf.keras.layers.ReLU))
        self.model.add(tf.keras.layers.BatchNormalization())
        self.model.add(tf.keras.layers.Conv2DTranspose(filters = 3, kernel_size = 5, stride = 2, padding = "same",
    ...
    def loss_function(self, disc_fake_output):
        ...
        return self.loss(disc_fake_output, tf.ones_like(disc_fake_output))
...
class Discriminator_Model(tf.keras.Model):
    def __init__(self):
        ...
        self.model.add(tf.keras.layers.Conv2D(filters=3, kernel_size=5, stride=2, padding="same",
                                              activation=tf.keras.layers.LeakyReLU(0.2)))
        self.model.add(tf.keras.layers.Conv2D(filters=64, kernel_size=5, stride=2, padding="same",
                                              activation=tf.keras.layers.LeakyReLU(0.2)))
        self.model.add(tf.keras.layers.BatchNormalization())
        self.model.add(tf.keras.layers.Conv2D(filters=128, kernel_size=5, stride=2, padding="same",
                                              activation=tf.keras.layers.LeakyReLU(0.2)))
        self.model.add(tf.keras.layers.BatchNormalization())
        self.model.add(tf.keras.layers.Conv2D(filters=256, kernel_size=5, stride=2, padding="same",
                                              activation=tf.keras.layers.LeakyReLU(0.2)))
        self.model.add(tf.keras.layers.BatchNormalization())
    ...
    def loss_function(self, disc_real_output, disc_fake_output):
        ...
        """
        # TODO: Calculate the loss
        return self.loss(disc_real_output, tf.ones_like(disc_real_output)) + self.loss(disc_fake_output, tf.zeros_like(disc_fake_output))
        #sum loss real (onelike) fake (zerolike)
...
def train(generator, discriminator, dataset_iterator, manager):
    ...
    for iteration, batch in enumerate(dataset_iterator):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            noise = tf.random.uniform([args.batch_size, args.z_dim], -1, 1)
        ...
        g_grads = tape1.gradient(g_loss, generator.trainable_variables)
        d_grads = tape2.gradient(d_loss, discriminator.trainable_variables)
        ...
        #2 tapes?? Persistent as true??
            #use generator to create noise?
...
def test(generator):
    ...
    z = tf.random.uniform([10, args.z_dim], -1,1)
    img = generator(z).numpy()

    ### Below, we've already provided code to save these generated images to files on disk
    # Rescale the image from (-1, 1) to (0, 255)
    img = ((img / 2) - 0.5) * 255 # maybe change - to +
    # Convert to uint8
